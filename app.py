...
class OptionStrategyBot:
    ...
    def __init__(self, api_key=None, secret=None, quote_currency="USDT", r=0.01):
        ...
        self.iv_threshold = 0.50  # Limiar para volatilidade atrativa
            ...
            for asset, strategies in results.items():
                ...
                for strat_name, (signal, roll_instruction) in strategies.items():
                    ...
                    if "No Trade" not in signal["strategy"] and "Erro" not in signal["strategy"]:
                        signal_id = db.insert_signal(asset, signal["strategy"], signal.get("expiration", ""),
                                                     signal.get("premium", 0), signal, roll_instruction)
                        if signal_id is not None:
                            db.insert_signal_legs(signal_id, signal, bot.default_qty)
        except Exception as e:
